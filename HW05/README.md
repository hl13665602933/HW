...
CNN and LeNet-5 MNIST Handwritten Digit Recognition ProjectCNN 和 LeNet-5 MNIST 手写数字识别项目

CNN 和 LeNet-5 MNIST 手写数字识别项目The CNN and LeNet-5 MNIST Handwritten Digit Recognition Project
...
bash
运行
python simple_cnn.py   运行 Python 脚本 simple_cnn.pyRun the Python script simple_cnn.py by typing "python simple_cnn.py" in the command line.在命令行中输入“python simple_cnn.py”即可运行 Python 脚本 simple_cnn.py 。
运行 LeNet-5：
bash
运行
python train_lenet.py   运行 Python 脚本 train_lenet.py 。Run the Python script train_lenet.py by executing "python train_lenet.py".运行 Python 脚本 train_lenet.py ，执行“python train_lenet.py”。
## 输出文件
模型权重：simple_cnn_mnist.pth、lenet5_mnist.pthModel weights: simple_cnn_mnist.pth, lenet5_mnist.pthModel weights: simple_cnn_mnist.pth, lenet5_mnist.pthModel weights: simple_cnn_mnist.pth, lenet5_mnist.pth
